...
public class FillProblems {
	...
	private List<User> getFas(Connection c, String fa) throws SQLException {
		...
		if (!Strings.isNullOrEmpty(fa)) {
			...
			for (String user : fa.split(splitter)) {
				user = user.trim();
				int id = -1;
				List<User> users = Server.getDao().getUserSearch(c, AUTH_USER_ID, user);
				if (!users.isEmpty()) {
					id = users.get(0).id();
				}
	...
	private void insertProblem(Connection c, int idArea, int idSector, Data d) throws IOException, SQLException, NoSuchAlgorithmException, InterruptedException, ParseException, ImageReadException, ImageWriteException {
		logger.debug("insert {}", d);
		List<User> fa = getFas(c, d.getFa());
		Type t = Server.getDao().getTypes(c, REGION_ID).stream().filter(x -> x.id() == d.getTypeId()).findFirst().get();
		...
		Server.getDao().setProblem(c, AUTH_USER_ID, setup, p, null);
	}

	private int upsertArea(Connection c, Data d) throws IOException, SQLException, NoSuchAlgorithmException, InterruptedException, ImageReadException, ImageWriteException, ParseException {
		for (Area a : Server.getDao().getAreaList(c, AUTH_USER_ID, REGION_ID)) {
