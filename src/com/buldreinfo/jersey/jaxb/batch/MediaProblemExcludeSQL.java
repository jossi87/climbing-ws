...
/*
SELECT CONCAT('INSERT INTO media_problem_exclude (media_id, problem_id) VALUES (',ms.id,',',p.id,'); -- ',CONCAT(r.url,'/problem/',p.id),' - ',a.name,' - ',s.name,' - ',p.name) note
...
GROUP BY ms.id, p.id, r.url, a.name, s.name, p.name
