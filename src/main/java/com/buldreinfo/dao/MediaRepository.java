...
public class MediaRepository {
	...
	@Transactional(readOnly = true)
	public List<Media> getMediaProblemCombined(Optional<Integer> authUserId, int sectorId, int problemId) {
		var sql = """
				WITH req AS (
				    SELECT ? auth_user_id, ? sector_id, ? problem_id
				)
				SELECT m.id, m.uploader_user_id, UNIX_TIMESTAMP(m.updated_at) version_stamp, mma.focus_x, mma.focus_y, mma.primary_color_hex media_primary_color_hex,
				       m.description, m.width, m.height, m.is_movie, m.suffix, m.is_360, m.embed_url, m.thumbnail_seconds,
				       m.date_created, m.date_taken, COALESCE(mp.milliseconds, 0) milliseconds,
				       ph.id photographer_id, TRIM(CONCAT(ph.firstname, ' ', COALESCE(ph.lastname,''))) photographer_name,
				       (
				           SELECT JSON_ARRAYAGG(JSON_OBJECT('id', tu.id, 'name', TRIM(CONCAT(tu.firstname, ' ', COALESCE(tu.lastname,'')))))
				           FROM media_user tmu
				...
				ORDER BY CASE WHEN ms.media_id IS NOT NULL THEN 0 ELSE 1 END, -ms.sorting DESC, m.is_movie, m.embed_url, IFNULL(mp.pitch,0), -mp.sorting DESC, m.id
				""";
		var res = jdbcClient.sql(sql)
				.params(authUserId.orElse(0), sectorId, problemId)
				.query((rs, _) -> {
					var embedUrl = rs.getString("embed_url");
					long millis = rs.getLong("milliseconds");
					if (embedUrl != null && millis > 0) {
		...
		return jdbcClient.sql(sql)
				...
				.query((rs, _) -> {
					...
					return new Media(
							...
							m.dateCreated(), m.dateTaken(), m.photographer(), m.tagged(), m.description(),
							m.mediaSvgs(), m.svgProblemId(), m.svgs(), m.embedUrl(), m.thumbnailSeconds(),
							false, m.areas(), m.sectors(), m.problems(), m.trails(), m.guestbookId(), m.userAvatarId()
							);
				}).list();
	}
