...
public class ImageClassifierService {
	public record AnalysisResult(String hexColor, List<MediaLabel> labels, List<MediaObject> objects) {}
	public record MediaLabel(String description, float score) {}
	...
	private List<MediaLabel> parseLabels(JsonNode root) {
		List<MediaLabel> labels = new ArrayList<>();
		for (JsonNode node : root.path("labelAnnotations")) {
			labels.add(new MediaLabel(
