...
public class WithoutJsController extends BaseController {
    ...
    @Operation(summary = "Get area by id without JavaScript")
    ...
    public ResponseEntity<String> getWithoutJsProblem(HttpServletRequest request,
            @PathVariable int id,
            @PathVariable(required = false, name = "mediaId") Integer mediaId,
            @PathVariable(required = false) Integer pitch) throws Exception {
        if (id <= 0) return createBadRequestResponse("Invalid id=" + id);
    ...
    @Operation(summary = "Get sector by id without JavaScript")
